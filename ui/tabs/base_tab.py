...
class BaseTab(ABC):
    ...
    def _set_status(self, text, color='blue', timeout=0):
        """Push a status message to the main window status bar."""
        from ui.ui_constants import show_status
        prefix = self._STATUS_PREFIX_MAP.get(
            (self.diagnostic_name, self.tab_type), self.diagnostic_name)
        show_status(self.frame, prefix, text, color, timeout)
