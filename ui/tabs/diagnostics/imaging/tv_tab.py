...
from data_loaders.tv_loader import (
    TV_FPS, TV_OFFSET, get_tv_zip_path, find_available_tvs
)
from ui.tabs.diagnostics.imaging.tv_utils import frame_to_time, time_to_frame
...
class TVTab:
    ...
    def _search_available_tvs(self):
        ...
        if not available_tvs:
            ...
            self.tv_dropdown.addItem('-- Select --')
            self._set_status("No TV files found", 'red')
            return
        ...
        self._set_status(f"Found: {', '.join(available_tvs)} for #{shot_number}", 'green')
    ...
    def _load_compare_mode(self, tv1_path, tv2_path):
        ...
        # Update UI
        self._set_status(f"TV01: {len(self.tv1_images)} frames, TV02: {len(self.tv2_images)} frames", 'green')
    ...
    def _set_status(self, text, color='blue', timeout=0):
        ...
        show_status(self.frame, "TV", text, color, timeout)
    ...
    def _load_zip_from_path(self, file_path):
        ...
        self._set_status(f"Loaded: {file_path.split('/')[-1]}", 'green')
        self.frame_slider.setMaximum(self.total_frames - 1)
        self.frame_total_entry.setReadOnly(False)
        self.frame_total_entry.setText(str(self.total_frames))
        self.frame_total_entry.setReadOnly(True)
