...
def _load_efit_mds(shot, tree, mds_ip):
    ...
    Returns dict with keys:
        ...
        r_outboard: (nt, nw) LFS midplane R for each psi_N (if 2D data available)
    """
    ...
    try:
        ...
        # Load 2D fields for R(psi_N) conversion
        r_outboard = None
        try:
            r_grid = np.array(mds.get('\\r').data(), dtype=float)
            z_grid = np.array(mds.get('\\z').data(), dtype=float)
            psirz = np.array(mds.get('\\psirz').data(), dtype=float)
            ssimag = np.array(mds.get('\\ssimag').data(), dtype=float)
            ssibry = np.array(mds.get('\\ssibry').data(), dtype=float)
            rmaxis = np.array(mds.get('\\rmaxis').data(), dtype=float)
            zmaxis = np.array(mds.get('\\zmaxis').data(), dtype=float)
            r_outboard = _compute_r_outboard(
                r_grid, z_grid, psirz, ssimag, ssibry, rmaxis, zmaxis, psin)
        except Exception as e:
            print(f"[EFIT] R-axis unavailable (no 2D data): {e}")
    finally:
...
def _compute_r_outboard(r_grid, z_grid, psirz, ssimag, ssibry,
                        rmaxis, zmaxis, psin_target):
    """Compute LFS midplane R for each psi_N value, per timepoint.

    Returns array of shape (n_time, len(psin_target)) or None on failure.
    """
    nt = psirz.shape[0]
    n_psin = len(psin_target)
    r_out = np.full((nt, n_psin), np.nan)

    for t in range(nt):
        try:
            # Interpolate psi at z = zmaxis[t] along R
            # psirz shape: (nt, nz, nr)
            psi_2d = psirz[t]
            z_idx_lo = np.searchsorted(z_grid, zmaxis[t]) - 1
            z_idx_lo = max(0, min(z_idx_lo, len(z_grid) - 2))
            f = (zmaxis[t] - z_grid[z_idx_lo]) / (z_grid[z_idx_lo + 1] - z_grid[z_idx_lo])
            psi_z0 = (1 - f) * psi_2d[z_idx_lo] + f * psi_2d[z_idx_lo + 1]

            denom = ssibry[t] - ssimag[t]
            if abs(denom) < 1e-12:
                continue
            psin_z0 = (psi_z0 - ssimag[t]) / denom

            # Restrict to LFS (R >= rmaxis). Anchor psi_N=0 at the magnetic
            # axis so that psin_target=0 maps exactly to rmaxis.
            lfs_mask = r_grid > rmaxis[t]
            r_lfs = np.concatenate(([rmaxis[t]], r_grid[lfs_mask]))
            psin_lfs = np.concatenate(([0.0], psin_z0[lfs_mask]))

            # Sort by psin to allow interp
            sort_idx = np.argsort(psin_lfs)
            psin_sorted = psin_lfs[sort_idx]
            r_sorted = r_lfs[sort_idx]

            r_out[t] = np.interp(psin_target, psin_sorted, r_sorted,
                                 left=rmaxis[t], right=np.nan)
        except Exception:
            continue
...
def _load_gfile(path):
    ...
    # GEQDSK scalar block layout (standard):
    # [0:5]:  rdim, zdim, rcentr, rleft, zmid
    # [5:10]: rmaxis, zmaxis, simag, sibry, bcentr
    rmaxis = scalars[5]
    zmaxis = scalars[6]
    simag = scalars[7]
    sibry = scalars[8]
    rgrid = rleft + np.arange(nw) * rdim / (nw - 1)
    zgrid = (zmid - zdim / 2.0) + np.arange(nh) * zdim / (nh - 1)
    psirz = psirz_flat.reshape((nh, nw))[None, :, :]  # (1, nh, nw)
    r_outboard = _compute_r_outboard(
        rgrid, zgrid, psirz,
        np.array([simag]), np.array([sibry]),
        np.array([rmaxis]), np.array([zmaxis]), psin)
...
class EfitProfileTab:
    ...
    def _create_load_section(self, parent):
        ...
        # "or" divider between Fetch and Open File
        ...
        grid.addWidget(gfile_btn, 2, 0, 1, 5)
    ...
    def _create_plot_controls(self, parent):
        ...
        # X-axis radios (psiN default; R uses LFS midplane mapping)
        ...
        # Rational surface toggle (only effective for QPSI)
        from ui.widgets.toggle_switch import ToggleSwitch
        sep = QFrame(); sep.setFrameShape(QFrame.HLine); sep.setFrameShadow(QFrame.Sunken)
        layout.addWidget(sep)
        rs_row = QHBoxLayout()
        self.rational_toggle = ToggleSwitch()
        self.rational_toggle.setChecked(getattr(self, '_show_rational', False),
                                        animate=False)
        self.rational_toggle.toggled.connect(self._on_rational_toggle)
        rs_row.addWidget(self.rational_toggle)
        rs_row.addWidget(QLabel("Show Rational Surfaces (QPSI)"))
        rs_row.addStretch()
        layout.addLayout(rs_row)

        parent.layout().addWidget(group)

    def _on_rational_toggle(self, checked):
        self._show_rational = bool(checked)
        # Re-plot if QPSI is currently selected
        var_name = self.var_combo.currentData() if hasattr(self, 'var_combo') else None
        if var_name == 'QPSI':
            self._plot()
    ...
    def _fetch(self):
        ...
        self._update_status()
        # Show only the just-fetched dataset in Available list
        # (cache is preserved so previously selected entries still work)
        self.available_listbox.clear()
        if key in self._efit_cache:
            efit = self._efit_cache[key]
            for t in efit['time']:
                ms = int(round(t * 1000))
                self.available_listbox.addItem(
                    f"{efit['shot']}_{ms:06d} ({efit['tree']})")
    ...
    def _on_gfiles_selected(self, paths):
        ...
        try:
            ...
            self._set_status(f"g-file error: {e}", 'red')
        finally:
            QApplication.restoreOverrideCursor()
    ...
    def _set_status(self, text, color='blue', timeout=0):
        ...
        show_status(self.frame, "EFIT Profile", text, color, timeout)

    def _update_status(self, color='green'):
        if not self._efit_cache:
            self._set_status("No EFIT data loaded", 'gray')
            return
    ...
    def _get_x_axis(efit, var_name, t_idx, xaxis_key):
        ...
        - R: LFS midplane R from r_outboard (computed from 2D psirz)
        """
        psin = efit['psin']
        ...
        if xaxis_key == 'R':
            r_out = efit.get('r_outboard')
            if r_out is not None:
                idx = min(t_idx, r_out.shape[0] - 1)
                return r_out[idx, :]
            return psin  # fallback
    ...
    def _plot(self):
        ...
        rational_overlays = []  # (x_plot, y_plot, color) for QPSI rational surface drawing
        ...
        for idx, entry in enumerate(entries):
            try:
                ...
                rational_overlays.append((x_plot, y_plot, colors[idx]))
            except Exception as e:
                print(f"[EFIT] Error plotting {entry}: {e}")

        # Rational surface markers (QPSI only)
        if var_name == 'QPSI' and getattr(self, '_show_rational', False):
            self._draw_rational_surfaces(ax, rational_overlays)
    ...
    # Common rational surfaces shown by default
    _RATIONAL_QS = [
        (1, 1), (3, 2), (2, 1), (5, 2), (3, 1),
        (7, 2), (4, 1), (5, 1),
    ]
    ...
    @staticmethod
    def _find_q_crossings(x_arr, q_arr, q_target):
        """Return x-values where q_arr crosses q_target (linear interpolation)."""
        crossings = []
        for i in range(len(q_arr) - 1):
            q0, q1 = q_arr[i], q_arr[i + 1]
            if np.isnan(q0) or np.isnan(q1):
                continue
            if (q0 - q_target) * (q1 - q_target) <= 0 and q0 != q1:
                # Linear interpolation
                t = (q_target - q0) / (q1 - q0)
                crossings.append(x_arr[i] + t * (x_arr[i + 1] - x_arr[i]))
        return crossings

    def _draw_rational_surfaces(self, ax, overlays):
        """Draw vertical markers at rational q surfaces (q = m/n) on QPSI plot."""
        if not overlays:
            return
        ymin, ymax = ax.get_ylim()

        for x_arr, q_arr, color in overlays:
            if len(q_arr) == 0:
                continue
            q_min, q_max = np.nanmin(q_arr), np.nanmax(q_arr)
            for m, n in self._RATIONAL_QS:
                q_target = m / n
                if q_target < q_min or q_target > q_max:
                    continue
                crossings = self._find_q_crossings(x_arr, q_arr, q_target)
                for x_c in crossings:
                    ax.axvline(x=x_c, color=color, linestyle='--',
                               linewidth=0.7, alpha=0.5)
                    ax.text(x_c, q_target, f' {m}/{n}',
                            fontsize=getattr(self, 'rational_fontsize', 9),
    ...
    def _show_style_dialog(self):
        ...
        for name, attr, lo, hi, default in [
            ...
            ("Rational surface font size", 'rational_fontsize', 4, 24, 9),
        ]:
            row = QHBoxLayout()
            row.addWidget(QLabel(name))
            spin = QSpinBox(); spin.setFixedWidth(W)
            spin.setRange(lo, hi); spin.setValue(getattr(self, attr, default))
    ...
    def _build_csv_lines(self):
        ...
        lines.append(f"#%10s,%10s,%10s,%10s,%10s,%10s,%10s,%14s\n" % (
            "Shot", "Tree", "Time[s]", "R[m]", "psi_N", "rho_pol", "rho_tor",
            f"{var_name}[{units}]"))
        ...
        for entry in entries:
            ...
            x_R = self._get_x_axis(efit, var_name, t_idx, 'R')
            ...
            for R, psi, rpol, rtor, yi in zip(x_R, x_psi, x_rpol, x_rtor, y):
                lines.append(" %10s,%10s,%10.4f,%10.6f,%10.6f,%10.6f,%10.6f,%14.6e\n" % (
                    shot, tree, t_actual, R, psi, rpol, rtor, yi))
    ...
    def _restore_settings(self):
        ...
        self.rational_fontsize = s.get("rational_fontsize", 9)
        self._show_rational = s.get("show_rational", False)
        # Sync toggle widget visual state with restored value
        if hasattr(self, 'rational_toggle'):
            self.rational_toggle.setChecked(self._show_rational, animate=False)
        ...
        saved_tree = s.get("efit_tree", "")
        if saved_tree and hasattr(self, 'tree_combo'):
            idx = self.tree_combo.findData(saved_tree)
            if idx >= 0:
    ...
    def save_settings(self):
        ...
        s["rational_fontsize"] = getattr(self, 'rational_fontsize', 9)
        s["show_rational"] = getattr(self, '_show_rational', False)
        s["variable"] = self.var_combo.currentData() or ""
        s["shot"] = self.shot_entry.text().strip()
        if hasattr(self, 'tree_combo'):
            s["efit_tree"] = self.tree_combo.currentData() or ""
