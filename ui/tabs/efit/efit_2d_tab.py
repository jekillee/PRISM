...
class Efit2DTab:
    ...
    def _create_plot_controls(self, parent):
        ...
        # Rational surface toggle (q = m/n contours)
        ...
        rs_row.addWidget(QLabel("Show Rational Surfaces"))
        rs_row.addStretch()
    ...
    def _on_rational_toggle(self, checked):
        ...
        # If toggle is enabled and any cached entry lacks QPSI profile,
        # re-fetch it so rational surfaces can be drawn.
        if self._show_rational:
            try:
                self._ensure_profiles_loaded()
            except Exception as e:
                print(f"[EFIT 2D] Could not refresh profiles: {e}")
        self._plot()

    def _ensure_profiles_loaded(self):
        """Re-fetch any cached MDS+ entries that don't have QPSI profile."""
        from data_loaders.efit_viewer_loader import load_efit_mds
        for key, efit in list(self._efit_cache.items()):
            tree = efit.get('tree') or ''
            if tree == 'gfile':
                continue
            profiles = efit.get('profiles') or {}
            if 'QPSI' in profiles:
                continue
            shot_str = efit.get('shot')
            try:
                shot_int = int(shot_str)
            except (TypeError, ValueError):
                continue
            print(f"[EFIT 2D] Re-fetching #{shot_int} ({tree}) for QPSI profile...")
            try:
                refreshed = load_efit_mds(shot_int, tree,
                                          load_scalars=False, load_profiles=True)
                # Preserve eq2d (original 2D fields); add profiles
                if refreshed.get('profiles'):
                    efit['profiles'] = refreshed['profiles']
            except Exception as e:
                print(f"[EFIT 2D]   Re-fetch failed: {e}")

    # Rational surface (q = m/n) values to display
    _RATIONAL_QS = [
        (1, 1), (3, 2), (2, 1), (5, 2), (3, 1),
        (7, 2), (4, 1), (5, 1),
    ]

    def _draw_rational_contours(self, ax, efit, t_idx, psi_N_2d,
                                 rgrid, zgrid, color):
        """Draw psi_N contours where q == rational m/n values."""
        profiles = efit.get('profiles') or {}
        qpsi_entry = profiles.get('QPSI')
        if qpsi_entry is None:
            print(f"[EFIT 2D] QPSI not loaded — re-fetch the shot with the toggle on, "
                  "or ensure load_profiles=True in efit_viewer_loader.")
            return
        # qpsi_entry can be a dict (from efit_viewer_loader) or ndarray
        if isinstance(qpsi_entry, dict):
            q_data = qpsi_entry.get('data')
            psin_grid = qpsi_entry.get('psin')
        else:
            q_data = np.asarray(qpsi_entry)
            psin_grid = None
        if q_data is None:
            print("[EFIT 2D] QPSI data is empty")
            return
        q_data = np.asarray(q_data)
        if q_data.ndim > 1:
            qi = min(t_idx, q_data.shape[0] - 1)
            q_arr = q_data[qi]
        else:
            q_arr = q_data
        q_arr = np.asarray(q_arr, dtype=float)
        if psin_grid is None or len(psin_grid) != len(q_arr):
            psin_grid = np.linspace(0.0, 1.0, len(q_arr))
        psin_grid = np.asarray(psin_grid, dtype=float)

        q_finite = q_arr[np.isfinite(q_arr)]
        if q_finite.size == 0:
            print("[EFIT 2D] QPSI has no finite values")
            return
        q_min = float(np.min(q_finite))
        q_max = float(np.max(q_finite))
        ...
        for m, n in self._RATIONAL_QS:
            ...
            if q_t < q_min or q_t > q_max:
                continue
            # Find psi_N where q crosses q_t (linear interp on first crossing)
            psin_cross = None
            for i in range(len(q_arr) - 1):
                q0, q1 = q_arr[i], q_arr[i + 1]
                if not (np.isfinite(q0) and np.isfinite(q1)):
                    continue
                if (q0 - q_t) * (q1 - q_t) <= 0 and q0 != q1:
                    f = (q_t - q0) / (q1 - q0)
                    psin_cross = psin_grid[i] + f * (psin_grid[i + 1] - psin_grid[i])
                    break
            if psin_cross is None or not (0 <= psin_cross <= 1.05):
                continue

            cs = ax.contour(rgrid, zgrid, psi_N_2d, levels=[float(psin_cross)],
                            colors=[color], linewidths=0.9,
                            linestyles='dashed', alpha=0.85)
            try:
                ax.clabel(cs, inline=True, fmt=f'{m}/{n}',
                          fontsize=getattr(self, 'rational_fontsize', 9))
            except Exception:
                pass
            drawn += 1
        print(f"[EFIT 2D] Rational surfaces: drew {drawn} contour(s) "
              f"(q range {q_min:.2f}–{q_max:.2f})")
    ...
    def _fetch(self):
        ...
        if key in self._efit_cache:
            ...
            try:
                from data_loaders.efit_viewer_loader import load_efit_mds
                # Load profiles too so QPSI is available for rational surface markers
                efit_data = load_efit_mds(int(shot), tree, load_scalars=False, load_profiles=True)
        ...
        if key in self._efit_cache:
            ...
            if time_arr is not None:
                shot_str = efit.get('shot', '?')
                tree_str = efit.get('tree', '?')
                for t in time_arr:
                    ms = int(round(t * 1000))
                    self.available_listbox.addItem(
                        f"{shot_str}_{ms:06d} ({tree_str})")
    ...
    def _set_status(self, text, color='blue', timeout=0):
        ...
        show_status(self.frame, "EFIT 2D", text, color, timeout)
    ...
    def _update_status(self, color='green'):
        ...
        self._set_status(
            f"{n_runs} run(s) loaded  \u2014  {total_t} time points", color)
    ...
    def _open_browse(self):
        ...
        class _Efit2DBrowse(_PreviewBase):
            ...
            def _update_plot(s, t_idx):
                ...
                n = s._n_contours
                levels = (np.linspace(0, 1, n + 2)[1:-1]
                ...
                # Fix x/y limits to the (R, Z) grid range, then enforce equal
                # aspect by adjusting the BOX (not data limits), so xlim/ylim
                # don't drift between frames.
                ax.set_xlim(float(rgrid[0]), float(rgrid[-1]))
                ax.set_ylim(float(zgrid[0]), float(zgrid[-1]))
    ...
    def _plot(self):
        ...
        # Interior contour levels (strictly between 0 and 1; exclude separatrix)
        if n_contours > 0:
            interior_levels = np.linspace(0, 1, n_contours + 2)[1:-1]
        else:
            interior_levels = np.array([])
        ...
        for idx, entry in enumerate(entries):
            ...
            # Convert contour grid based on type. Levels remain at the same
            # numeric values (0.1, 0.2, ...) but the field is the chosen
            # coordinate, so contours occupy different physical locations
            # depending on coordinate choice.
            ...
            # Rational surface contours (q = m/n) drawn on the psi_N field
            if getattr(self, '_show_rational', False):
                try:
                    self._draw_rational_contours(
                        ax, efit, t_idx, psi_N, rgrid, zgrid, color)
                except Exception as e:
                    print(f"[EFIT 2D] Rational surface draw failed for {entry}: {e}")
