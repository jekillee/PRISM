...
class EfitPfileTab:
    ...
    def _on_pfiles_selected(self, paths):
        ...
        try:
            ...
            self._set_status(f"p-file error: {e}", 'red')
        finally:
    ...
    def _set_status(self, text, color='blue', timeout=0):
        ...
        show_status(self.frame, "EFIT p-File", text, color, timeout)

    def _update_status(self, color='green'):
        if not self._pfile_cache:
            self._set_status("No p-file loaded", 'gray')
            return
        n_runs = len(self._pfile_cache)
        n_v = max(len(p['profiles']) for p in self._pfile_cache.values())
        self._set_status(f"{n_runs} p-file(s) loaded  \u2014  {n_v} variables", color)
