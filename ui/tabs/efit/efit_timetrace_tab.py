...
class EfitScalarTab:
    ...
    def _create_load_section(self, parent):
        ...
        grid.addWidget(open_btn, 2, 0, 1, 5)
    ...
    def _fetch(self):
        ...
        self._set_status(f"Loading #{shot_number} ({tree})...", 'blue')
    ...
    def _set_status(self, text, color='blue', timeout=0):
        ...
        show_status(self.frame, "EFIT Time Trace", text, color, timeout)

    def _update_load_status(self, color='green'):
        if not self._data_cache:
            self._set_status("No data loaded", 'gray')
            return
        n_runs = len(self._data_cache)
        n_v = max(len(d.get('scalars', {})) for d in self._data_cache.values())
        self._set_status(f"{n_runs} run(s) loaded  —  {n_v} variables", color)
    ...
    def _restore_settings(self):
        ...
        if saved_tree and hasattr(self, 'tree_combo'):
            self.tree_combo.setCurrentText(saved_tree)
    ...
    def save_settings(self):
        ...
        if hasattr(self, 'tree_combo'):
            s["efit_tree"] = self.tree_combo.currentText()
