...
class NeTeProfileTab(ProfileBaseTab):
    ...
    def _get_efit_profile_data(self, entry, interp_func, dt_s=0.0):
        ...
        if dt_s > 0:
            ...
            if n_avg > 0:
                Te_profile = np.nanmean(Te_all[:, t_mask], axis=1)
                Te_err = np.nanmean((Te_eu_all[:, t_mask] + Te_el_all[:, t_mask]) / 2.0, axis=1) / np.sqrt(n_avg)
                ne_profile = np.nanmean(ne_all[:, t_mask], axis=1)
                ne_err = np.nanmean((ne_eu_all[:, t_mask] + ne_el_all[:, t_mask]) / 2.0, axis=1) / np.sqrt(n_avg)
                ...
                if source in ['TS', 'TS+ECE']:
                    ...
                    if dt_s > 0:
                        ...
                        if n_avg > 0:
                            ...
                            Te_err_upper_profile = np.nanmean(Te_err_upper[:, t_mask], axis=1) / np.sqrt(n_avg)
                            Te_err_lower_profile = np.nanmean(Te_err_lower[:, t_mask], axis=1) / np.sqrt(n_avg)
                            ne_profile = np.nanmean(ne_data[:, t_mask], axis=1)
                            ne_err_upper_profile = np.nanmean(ne_err_upper[:, t_mask], axis=1) / np.sqrt(n_avg)
                            ne_err_lower_profile = np.nanmean(ne_err_lower[:, t_mask], axis=1) / np.sqrt(n_avg)
                        else:
                            time_idx = np.argmin(np.abs(data.time - time_point))
                            Te_profile = Te_data[:, time_idx]
                            Te_err_upper_profile = Te_err_upper[:, time_idx]
                    ...
                    if dt_s > 0:
                        label = f'#{shot_number} {time_ms}ms\u00b1{dt_ms:.0f}ms (TS)'
                    else:
